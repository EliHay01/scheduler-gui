...
function RegisterPilot(props) {
  ...
  const initUsageDetails = { flightDate: '', startHobbs: '', endHobbs: '', totalHobbsTime:'', totalPrice:'', 
                            endTach: '', takeOffAirport:'', intermidiateAirport: '', landAirport:'' }
  const [usageDetails, setUsageDetails] = useState(initUsageDetails);
  ...
  useEffect(() => {
    ...
    if (props.user && props.user.pilotExist){
      UserService.getUsageDetails().then(
        response => {
  ...
  return (
    <Formik
      ...
      onSubmit={onSubmit}     
      initialValues={{        
        name: usageDetails.name,
        emailAddress: usageDetails.emailAddress,
        phoneNumber: usageDetails.phoneNumber,
        licenseType: usageDetails.licenseType,
        medicalClass: usageDetails.medicalClass,
        medicalExpirationDate: usageDetails.medicalExpirationDate,
        imageSrc: imageSrc
      }}
